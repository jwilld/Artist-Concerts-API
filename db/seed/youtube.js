...
let youtubeList = []
    ...
    Artist.find({}).then(artists => artists.forEach(artist =>{
        ...
        let url = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=${apiKeyFive}`
        ...
        .then(()=> fs.writeFile('youtube-ids-2.json',JSON.stringify(youtubeList),(err)=>{
            if(err){
                console.log(err)
            }
